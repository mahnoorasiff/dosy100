...
const removeComment = (imageId, commentId) =>{
  ...
  firebase.database().ref('/comments/'+imageId+'/'+commentId).remove()
  ...
  const rendering = ({ item} ) => {
    ...
    return (
      ...
      <View style={styles.content}>
        <View style={{...styles.contentHeader, flexDirection:"row"}}>
          <Text  style={styles.name}>{item.author}</Text>  
          <Text style={styles.time}>
            {timeConverter(item.posted)}
          </Text> 
        </View>
    ...
    <View style={styles.contentHeader}>
        ...
        <Ionicons style={{}} name="trash-outline" size= {15}
                      onPress={() => removeComment(photoid, item.id)} />
    </View>
      </View>
    </View>
)}
